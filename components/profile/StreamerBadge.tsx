...
export function StreamerBadge({ compact = false, href }: { compact?: boolean; href: string }) {
  const { t } = useI18n()
  const reduce = useReducedMotion()
  const hints = t.profile.streamerHints
  ...
  const className = cn('dev-badge streamer-badge shrink-0', compact && 'is-compact', hot && 'is-hot')
  const tipNode = (
    <AnimatePresence>
      {hot ? (
        <motion.i
